...
h=0; % es h/c
p=0;  % es p/c
alpha=3.5*pi/180; % ángulo de ataque
...
Nplot=7;
Xplot=[0.07 0.18 0.27 0.38 0.4621 0.9 1];
icont=1;
...
if X(n)>= Xplot(icont) || un(2)<=0 
     plot(un,eta)
     legend('X=0.07','X=0.18','X=0.27','X=0.38','X_s=0.4621')
     title('Perfil de velocidades para el extradós')
...
figure(2)
plot(Xext,Yext,'b',Xint,Yint,'b','LineWidth',2)
hold on
